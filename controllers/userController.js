...
var { error, sucess } = require('consola');
...
module.exports = {
  ...
  resetPassword: async function (req, res) {
    ...
    await UserModel.findOne({ email: email }).exec((err, userByemail) => {
      if (err) {
        res.status(500).json({
          message: err.message,
          status: 500,
       ...
       var sent=  transporter.sendMail(
          {
            to: email,
            subject: "Reset password for" + " " +UserModel.name,
            text: "bonjour mr",
            html: `<!DOCTYPE html>
            <html>
            <head>
            ...
            <p> <b>Your new password is:</b>${newToken} </p>
            <br> try it know!!
            <a href="http://localhost:3000/user/updateUser/${newToken }">Verify email </a>
              <br>
            </body>
            </html>`,
          },
        ...
        if (sent != '0') {
          ...
          UserModel.findOneAndUpdate({ email }, data).exec((err, userUpdate) => {
           
            res.status(200).json({
              status: 200,
              message: "password is updated with success !",
              data: userUpdate,
            });
